		...
		if (not teleportedServers) and (not shared.VapeIndependent) and vape.AutoTeleport.Enabled then
			...
			local teleportScript = [[
				if shared.VapeDeveloper then
					...
					loadstring(readfile('catvape_ppsnip/init.lua'), 'init')()
				else
...
if not isfile('catvape_ppsnip/profiles/gui.txt') then
	writefile('catvape_ppsnip/profiles/gui.txt', 'new')
end
local gui = 'new'--readfile('catvape_ppsnip/profiles/gui.txt')

if not isfolder('catvape_ppsnip/assets/'..gui) then
	makefolder('catvape_ppsnip/assets/'..gui)
end
vape = loadstring(downloadFile('catvape_ppsnip/guis/'..gui..'.lua'), 'gui')()
shared.vape = vape
_G.vape = vape
...
if not shared.VapeIndependent then
	loadstring(downloadFile('catvape_ppsnip/games/universal.lua'), 'universal')()

	local found = false
	local callback = shared.VapeDeveloper and readfile or downloadFile
	
	for i, v in httpService:JSONDecode(callback('catvape_ppsnip/profiles/supported.json')) do
		...
		if game.GameId == v.gameid then
			for i2, v2 in v do
				if typeof(v2) == 'table' and table.find(v2.Ids, game.PlaceId) then
					found = true
					vape.Place = v2.Place
					if not isfolder('catvape_ppsnip/games/'.. i) then
						makefolder('catvape_ppsnip/games/'.. i)
					end
					
					loadstring(callback('catvape_ppsnip/games/'.. i.. '/'.. i2.. '.luau'), tostring(game.PlaceId))(...)
					loadstring(callback('catvape_ppsnip/games/'.. i.. '/'.. 'premium'.. '.luau'), 'paid '.. tostring(game.PlaceId))(...)
					break
				end
			end
		end
	...
	if not found then
		local suc, res = pcall(function()
			return not shared.VapeDeveloper and game:HttpGet('https://raw.githubusercontent.com/ppsnip/Catvape-mine/'..readfile('catvape_ppsnip/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true) or '404: Not Found'
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('catvape_ppsnip/games/'..game.PlaceId..'.lua'), tostring(game.PlaceId))(...)
		end
	end
