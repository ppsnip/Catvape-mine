...
run(function()
	...
	local ProjectileNoYPred
	local ProjectileAntiGround
	...
	local function solveProjectile(origin, ent, targetPart)
		...
		if ProjectileNoYPred.Enabled then
			velocity = Vector3.new(velocity.X, 0, velocity.Z)
		end
		local calc = prediction.SolveTrajectory(origin, ProjectileSpeed.Value, ProjectileGravity.Value, targetPart.Position, velocity, workspace.Gravity, ent.HipHeight, nil, ProjectileRaycast)
		if not calc then return end
		if ProjectileAntiGround.Enabled then
			GroundCheckParams.FilterDescendantsInstances = {ent.Character}
			local groundRay = workspace:Raycast(calc, Vector3.new(0, -(ent.HipHeight + 0.5), 0), GroundCheckParams)
			if groundRay then return end
		end
		return calc
	end
	...
	Projectile = SilentAim:CreateToggle({
		...
		Function = function(callback)
			...
			ProjectileNoYPred.Object.Visible = callback
...
run(function()
	...
	FPSUnlocker = vape.Categories.Render:CreateModule({
		Name = 'FPS Unlocker',
		Tooltip = 'Removes the 60 FPS cap and sets a custom FPS limit',
		Disabled = not setfpscap,
		DisabledTooltip = 'Your executor does not support setfpscap',
		Function = function(call)
	...
	FPSCap = FPSUnlocker:CreateSlider({
		Name = 'FPS Cap',
		Min = 30,
		Max = 480,
		Decimal = 1,
